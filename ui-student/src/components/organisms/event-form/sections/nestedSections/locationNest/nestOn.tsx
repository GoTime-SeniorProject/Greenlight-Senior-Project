...
// ─── Local ────────────────────────────────────────────────────────────────────
import { useGetOnCampusQuery } from "../../../../../../generated";
...
export default function OnCampusSection({ control, setValue }: Props) {
    ...
    useEffect(() => {
        if (onCampusData?.getLocations) {
            setAllLocations(onCampusData.getLocations);
            if (onCampusData.getLocations.length < INITIAL_LIMIT) {
    ...
    const loadMoreLocations = useCallback(async () => {
        ...
        try {
            ...
            if (result.data?.getLocations) {
                const newLocations = result.data.getLocations;
