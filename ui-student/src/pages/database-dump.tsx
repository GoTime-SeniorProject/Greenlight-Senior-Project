import { useGetEventsQuery, useGetOrganizationsQuery, useGetUsersQuery, useGetPurchasesQuery } from '../generated';
import type { GetDbDumpQuery } from '../generated';
...
type EventRow = GetDbDumpQuery['getEvents'][number] & { key: string };
type UserRow = GetDbDumpQuery['getUsers'][number] & { key: string };
type OrgRow = GetDbDumpQuery['getOrganizations'][number] & { key: string };
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      <section className="mt-8">
        ...
        {eventsData?.getEvents?.length ? (
          (() => {
            ...
            const dataSource = eventsData!.getEvents.slice().sort((a: any, b: any) => Number(a.id) - Number(b.id)).map((e) => ({ ...e, key: e.id }));
      ...
      <section className="mt-8">
        ...
        {purchasesData?.getPurchases?.length ? (
          (() => {
            const eventMap = new Map<string, string>((eventsData?.getEvents ?? []).map((e: any) => [e.id, e.title]));
            ...
            const dataSource = purchasesData!.getPurchases.slice().sort((a: any, b: any) => Number(a.id) - Number(b.id)).map((p: any) => ({ ...p, key: p.id }));
      ...
      <section className="mt-8">
        ...
        {usersData?.getUsers?.length ? (
          (() => {
            ...
            const dataSource = usersData!.getUsers.slice().sort((a: any, b: any) => Number(a.id) - Number(b.id)).map((u: any) => ({ ...u, key: u.id }));
      ...
      <section className="mt-8">
        ...
        {orgsData?.getOrganizations?.length ? (
          (() => {
            ...
            const dataSource = orgsData!.getOrganizations.slice().sort((a: any, b: any) => Number(a.id) - Number(b.id)).map((o) => ({ ...o, key: o.id }));
